...
from neutele.simulation.utils import FirstOrderFilter
...
class MujocoBase:
    ...
    def __init__(self, model_path: str):
        ...
        self.data_loader = DataLoader(load_path="G://NEU_Tele/data/flying_hand_1767078030.h5")
    ...
    def run(self):
        ...
        for i in tqdm(range(len(self.data_loader) * int(round(0.01 / self.mj_model.opt.timestep)))):
            ...
            for name in self.cameras.keys():
                ...
                depth_img = image[f"{name}_depth"]
                depth_min = float(np.min(depth_img))
                depth_max = float(np.max(depth_img))
                if depth_max == depth_min:
                    depth_max = depth_min + 1.0
                depth_normalized = (depth_img - depth_min) / (depth_max - depth_min)
                depth_img = np.flipud((depth_normalized * 255).astype(np.uint8))
                cv2.imwrite(f"image/{name}/depth_{i:06d}.png", depth_img)
...
if __name__ == "__main__":
    ...
    agent = MujocoBase("../station/flying_hand/description/x500_arm_v2/x500_arm_v2.xml")
    # agent = MujocoBase("G:\\NEU_Tele\\neutele\\station\\flying_hand\\description\\x500_arm_v1\\x500_arm_v1.xml")
    # agent = MujocoBase("G:\\NEU_Tele\\neutele\\station\\flying_hand\\description\\x650_arm_v2\\x650_arm_v2.xml")
    try:
        agent.run()
