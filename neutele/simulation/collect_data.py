...
from neutele.simulation.network import PublisherServer
...
class MujocoBase:
    ...
    def __init__(self, model_path: str):
        ...
        self.policy = Policy("weight/policy.onnx")
        ...
        self.obs_data = {
            ...
            "servo_velocity": np.zeros(5),  # Servo joint velocities
            "last_action": np.zeros(4),  # Previous control action
            "command": np.zeros(4),  # Teleoperation command
        }
        self.command = np.zeros(4)  # Current teleoperation command
    ...
    def get_command(self) -> np.ndarray:
        ...
        """
        channel = self.tele_core.act()
        self.command[:2] = channel[:2]  # XY position commands
        self.command[2] = 0.75 * channel[4]  # Z position command (scaled)
        self.command[3] = 0.5 * (channel[2] - channel[3])  # Yaw command from differential input
        return self.command
    ...
    def control(self):
        ...
        self.obs_data["servo_velocity"] = data["servo_velocity"]
        self.obs_data["command"] = self.get_command()
        ...
        # action = np.concatenate([rotor_velocity_action / 10, rotor_thrust_action, servo_action])
