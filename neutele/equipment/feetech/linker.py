...
class Linker(BaseEquipment):
    ...
    def calibrate(self, ids: Sequence[int], home_poses: Sequence[int]):
        home_poses = np.array(home_poses) * 2048.0 / np.pi
        self._driver.calibrate(ids, home_poses)

    def act(self) -> Sequence[float]:
        pos, _ = self._driver.get_pos_and_vel()
        pos = np.array([v for k, v in pos.items() if k in self._ids]) * np.pi / 2048.0
        return pos
