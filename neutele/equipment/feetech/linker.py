...
class Linker(BaseEquipment):
    def __init__(self, config: Dict[str, Any], driver: Optional[FeeTechDriver] = None):
        ...
        if self._dynamic_enable:
            ...
            self._torque_current_mapping = np.array(
                [-1.0 / 0.0981 / KT_MAPPING[servo] * 1000.0 / 6.5 for servo in self._servo_types[:4]]
            )

            self._external_torque = np.zeros_like(self._dof)

            urdf_model_path = os.path.abspath("G:\\NEU_Tele\\neutele\\station\\flying_hand\\urdf\\leader\\leader.urdf")
            ...
            self.null_space_joint_target = [0.0, 1.5708, 0.0, 0.0, 0.0]
    ...
    def act(self) -> Tuple[Sequence[float], Sequence[float]]:
        pos_raw, vel_raw = self._driver.get_pos_and_vel()
        pos = np.array([v for k, v in pos_raw.items()]) * self._signs * np.pi / 2048.0
        gripper_mask = np.isin(self._ids, self._gripper_ids)
        pos[gripper_mask] = 1.0 - np.clip(pos[gripper_mask] / (np.pi / 4.0), 0.0, 1.0)
        vel = np.array([v for k, v in vel_raw.items()]) * self._signs * 0.732 * np.pi / 30
        return pos, vel
    ...
    def _control_worker(self):
        while not self._stop_flag.is_set():
            ...
            self.set_torque(self._ids[:4], torque_arm[:4])
