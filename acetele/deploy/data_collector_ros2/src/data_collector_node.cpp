...
DataCollectorNode::DataCollectorNode() : Node("data_collector"), is_recording_(false), recorded_frame_count_(0)
{
    ...
    this->declare_parameter("color_topic", "/camera/front/color/image_raw");
    this->declare_parameter("depth_topic", "/camera/front/depth/image_rect_raw");
    this->declare_parameter("color_info_topic", "/camera/front/color/camera_info");
    this->declare_parameter("color_metadata_topic", "/camera/front/color/metadata");
    this->declare_parameter("depth_info_topic", "/camera/front/depth/camera_info");
    this->declare_parameter("depth_metadata_topic", "/camera/front/depth/metadata");
    this->declare_parameter("depth_to_color_ext_topic", "/camera/front/extrinsics/depth_to_color");
    this->declare_parameter("depth_to_depth_ext_topic", "/camera/front/extrinsics/depth_to_depth");
    ...
    std::string depth_metadata_topic = this->get_parameter("depth_metadata_topic").as_string();
    std::string depth_to_color_ext_topic = this->get_parameter("depth_to_color_ext_topic").as_string();
    std::string depth_to_depth_ext_topic = this->get_parameter("depth_to_depth_ext_topic").as_string();
    ...
    color_info_sub_ = this->create_subscription<sensor_msgs::msg::CameraInfo>(
        color_info_topic, qos, std::bind(&DataCollectorNode::color_info_callback, this, _1));
    ...
    depth_info_sub_ = this->create_subscription<sensor_msgs::msg::CameraInfo>(
        depth_info_topic, qos, std::bind(&DataCollectorNode::depth_info_callback, this, _1));

    depth_metadata_sub_ = this->create_subscription<realsense2_camera_msgs::msg::Metadata>(
        depth_metadata_topic, qos, std::bind(&DataCollectorNode::depth_metadata_callback, this, _1));

    depth_to_color_ext_sub_ = this->create_subscription<realsense2_camera_msgs::msg::Extrinsics>(
        depth_to_color_ext_topic, qos, std::bind(&DataCollectorNode::depth_to_color_ext_callback, this, _1));

    depth_to_depth_ext_sub_ = this->create_subscription<realsense2_camera_msgs::msg::Extrinsics>(
        depth_to_depth_ext_topic, qos, std::bind(&DataCollectorNode::depth_to_depth_ext_callback, this, _1));
...
void DataCollectorNode::get_latest_images(cv::Mat& color, cv::Mat& depth)
{
    ...
    if (!latest_color_.empty()) {
        latest_color_.copyTo(color);
    }
    if (!latest_depth_.empty()) {
        latest_depth_.copyTo(depth);
    }
}
...
void DataCollectorNode::save_data_worker()
{
    while (rclcpp::ok()) {
...

        {
            ...
            latest_color_.copyTo(color_snap);
            latest_depth_.copyTo(depth_snap);
        ...
        std::string depth_path = recording_dir_ + "/" + timestamp + "_depth.png"; // PNG for lossless depth
...
void DataCollectorNode::color_info_callback(const sensor_msgs::msg::CameraInfo::SharedPtr msg)
{
    (void)msg;
    update_status("color_info");
}
...
void DataCollectorNode::depth_info_callback(const sensor_msgs::msg::CameraInfo::SharedPtr msg)
{
    (void)msg;
    update_status("depth_info");
}

void DataCollectorNode::depth_metadata_callback(const realsense2_camera_msgs::msg::Metadata::SharedPtr msg)
{
    (void)msg;
    update_status("depth_metadata");
}

void DataCollectorNode::depth_to_color_ext_callback(const realsense2_camera_msgs::msg::Extrinsics::SharedPtr msg)
{
    (void)msg;
    update_status("depth_to_color");
}

void DataCollectorNode::depth_to_depth_ext_callback(const realsense2_camera_msgs::msg::Extrinsics::SharedPtr msg)
{
    (void)msg;
    update_status("depth_to_depth");
}
