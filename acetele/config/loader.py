...
class RobotSpecLoader:
    ...
    def _parse_end_effector(self, raw: Any, arm_path: str):
        ...
        if kind == "parallel_gripper":
            _reject_unknown(
                values,
                {"kind", "bus", "joint", "travel_range_rad", "kinematic_joint_names"},
                path,
            )
            raw_kinematic_joint_names = values.get("kinematic_joint_names", [])
            if not isinstance(raw_kinematic_joint_names, list):
                raise ValueError(
                    f"configuration field '{path}.kinematic_joint_names' must be an array of strings"
                )
            return ParallelGripperSpec(
                ...
                kinematic_joint_names=tuple(
                    _string_value(name, f"{path}.kinematic_joint_names[{index}]")
                    for index, name in enumerate(raw_kinematic_joint_names)
                ),
            )
