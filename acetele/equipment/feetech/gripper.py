...
FRAGILE_CLOSE_VELOCITY = 0.12
FRAGILE_OPEN_VELOCITY = 0.8
FRAGILE_CONTACT_VELOCITY_THRESHOLD = 0.03
...
class Gripper(BaseEquipment):
    def __init__(self, config: Dict[str, Any], driver: FeeTechDriver):
        ...
        self._fragile_close_velocity = FRAGILE_CLOSE_VELOCITY
        self._fragile_open_velocity = FRAGILE_OPEN_VELOCITY
        self._fragile_contact_velocity_threshold = FRAGILE_CONTACT_VELOCITY_THRESHOLD
    ...
    def set_fragile_position(self, command_position: float, state: Optional[GripperState] = None) -> bool:
        ...
        gripper_velocity = abs(float(state.velocity))
        ...
        if force_state.status == GripperForceStatus.HOLDING.value:
            ...
            if command_position > hold_position + self._fragile_release_deadband:
                self._contact_confirm_count = 0
                ...
                self.set_position(command_position, velocity=self._fragile_open_velocity)
                return True
            ...
            self.set_position(hold_position, velocity=self._fragile_close_velocity, torque=hold_torque)
            return True
        ...
        if (
            ...
            and measured_torque >= self._fragile_contact_torque_threshold_nm
            and gripper_velocity <= self._fragile_contact_velocity_threshold
        ):
        ...
        if self._contact_confirm_count >= self._fragile_contact_confirm_samples:
            ...
            force_state.hold_torque_nm = hold_torque
            self.set_position(current_position, velocity=self._fragile_close_velocity, torque=hold_torque)
            return True
        ...
        self.set_position(
            command_position,
            velocity=self._fragile_close_velocity,
            torque=self._fragile_max_hold_torque_nm,
        )
        return True
