...
XML_INSERT_AFTER_WORLD = """
    <actuator>
        ...
        <motor name="rotor_joint_thrust1" site="rotor_joint_thrust1"
               gear="0 0 1 0 0 -0.016" />
        <motor name="rotor_joint_thrust2" site="rotor_joint_thrust2"
               gear="0 0 1 0 0 0.016" />
        <motor name="rotor_joint_thrust3" site="rotor_joint_thrust3"
               gear="0 0 1 0 0 -0.016" />
        <motor name="rotor_joint_thrust4" site="rotor_joint_thrust4"
               gear="0 0 1 0 0 0.016" />
        <position name="joint_1" joint="joint_1" kp="748.6" kv="0.547"
                  forcerange="-4.905 4.905" ctrlrange="-2.6485 2.6485" />
        <position name="joint_2" joint="joint_2" kp="524.0" kv="0.727"
                  forcerange="-3.43 3.43" ctrlrange="0 3.4907" />
        <position name="joint_3" joint="joint_3" kp="524.0" kv="0.727"
                  forcerange="-3.43 3.43" ctrlrange="-2.6485 2.6485" />
        <position name="joint_4" joint="joint_4" kp="524.0" kv="0.727"
                  forcerange="-3.43 3.43" ctrlrange="-3.1416 3.1416" />
        <position name="joint_5" joint="joint_5" kp="212.6" kv="0.133"
                  forcerange="-1.3916 1.3916" ctrlrange="-1.723 0" />
        <position name="joint_gripper_left" joint="joint_gripper_left" kp="2000.0" kv="124.0"
                  forcerange="-49.06 49.06" ctrlrange="0 0.04225" />
        <position name="joint_gripper_right" joint="joint_gripper_right" kp="2000.0" kv="124.0"
    ...
    for rotor_name, site_name in rotor_sites.items():
        if f'site name="{site_name}"' not in content:
            ...
            if rotor_match:
                ...
                site_insert = (
                    f'\n                <site name="{site_name}" type="cylinder" size="0.01 0.005" '
                    f'pos="0 0 0" rgba="1 0 0 0.5"/>'
                )
...
if __name__ == "__main__":
    ...
    urdf_path = (
        BASE_DIR / ".." / "station" / "flying_hand" / "description" / target_name / f"{target_name}.urdf"
    ).resolve()
    xml_path = (
        BASE_DIR / ".." / "station" / "flying_hand" / "description" / target_name / f"{target_name}.xml"
    ).resolve()

    compile_urdf(str(urdf_path), str(xml_path))
