"""Pygame joystick polling with bounded startup and shutdown lifecycle."""
...
class JoystickDriver:
    ...
    def _worker(self):
        """Own pygame polling, reconnection, and publication of atomic snapshots."""
        ...
        try:
            ...
            while not self._stop_event.is_set():
                if not joystick:
                    # Reinitialize the joystick subsystem after unplug/replug; callers
                    # retain the same driver object while the worker reconnects.
                ...
                # Build a complete sample locally before acquiring the shared-data lock.
                # Readers see either the old frame or the new frame, never a partial one.
                mapped_data = {
                    "timestamp": time.time(),
                    "connected": True,
    ...
    def _cleanup_pygame(self):
        """Release pygame once even when startup failure and close race."""
    ...
    def _connect_joystick(self):
        """Return the first matching initialized joystick, skipping broken handles."""
        ...
        # Enumeration can retain stale handles after hot-plug. A failed candidate must
        # not prevent a later matching controller from connecting.
    ...
    def act(self) -> Union[Dict, None]:
        """Return an independent copy of the latest joystick snapshot."""

        with self._data_lock:
            return copy.deepcopy(self._latest_data)

    def is_connected(self) -> bool:
        """Return the worker's current device connection state."""

        with self._data_lock:
            return self._is_connected

    def close(self):
        """Stop the polling worker and release pygame within one second."""
...
class XboxDriver(JoystickDriver):
    ...
    def __init__(self, device_name_pattern="Xbox"):
        super().__init__(
            ...
            axis_map=self.default_axis_map,
