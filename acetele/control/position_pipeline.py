...
from acetele.config.specs import ControlSpec
...
class PositionControlPipeline:
    ...
    def apply(self, command: JointCommand, *, now_ns: int) -> JointCommand:
        ...
        target = _unwrap_near(command.positions, 0.5 * (self._lower + self._upper))
    ...
    def _adaptive_compensation(
        ...
        for index in range(self._count):
            ...
            if np.isnan(previous_target):
                ...
                target_delta = _wrap(float(target[index] - previous_target))
                ...
                if (
                    abs(
                        _wrap(
                            float(target[index] - self._learning_target[index])
                        )
                    )
            ...
            target_stable = (
                abs(_wrap(float(target[index] - self._stable_target[index])))
            ...
            error = _wrap(float(target[index] - measured[index]))
...
def _wrap(value: float) -> float:
    """Return the principal angular difference in ``[-pi, pi)``."""

    return (value + math.pi) % (2.0 * math.pi) - math.pi


def _unwrap_near(values: np.ndarray, references: np.ndarray) -> np.ndarray:
    """Choose each angle's equivalent turn nearest its physical-limit midpoint."""

    values = np.asarray(values, dtype=float)
    return values + 2.0 * math.pi * np.round((references - values) / (2.0 * math.pi))
