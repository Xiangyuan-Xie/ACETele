...
def root_rotation_matrix(quaternion: np.ndarray) -> np.ndarray:
    """
    Convert quaternions to 3x3 rotation matrices and return as flattened arrays.

    This function uses scipy's Rotation module to perform the conversion from
    quaternions to rotation matrices. The input quaternions are expected to
    be in scalar-first format (w, x, y, z).

    Parameters
    ----------
    quaternion : np.ndarray
        Input quaternion(s) in scalar-first format (w, x, y, z).
        Can be a single quaternion (4,) or multiple quaternions (n, 4).

    Returns
    -------
    np.ndarray
        Rotation matrices as flattened arrays. Returns shape (n, 9) where
        each row contains the flattened 3x3 rotation matrix in row-major order.

    Notes
    -----
    - The function always returns a 2D array even for single input.
    - Input quaternions are normalized by scipy's Rotation module.
    - The output matrices are proper rotation matrices (orthogonal with det=1).

    Examples
    --------
    >>> import numpy as np
    >>> quat = np.array([1, 0, 0, 0])  # Identity quaternion
    >>> result = root_rotation_matrix(quat)
    >>> result.shape
    (1, 9)
    >>> result[0]  # Flattened identity matrix
    ...
    # Create Rotation object from quaternions (scalar-first format)
    ...
    # Reshape to (n, 9) - flatten each 3x3 matrix
