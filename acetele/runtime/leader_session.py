...
class LeaderTeleopSession:
    ...
    def __init__(
        ...
        *,
        ...
        # FEETECH profiles encode acceleration explicitly during alignment. Other
        # protocols receive only the portable velocity constraint they advertise.
        bus_types = {bus.name: bus.type for bus in runtime.spec.buses}
        self._acceleration_groups = frozenset(
            arm.name
            for arm in runtime.spec.arms
            if runtime.spec.backend == Backend.PHYSICAL
            and bus_types[arm.bus]
            in (BusType.FEETECH_PACKET, BusType.FEETECH_MODBUS_RTU)
        )
    ...
    def observe_landed(self, landed: bool) -> bool:
        """Stop only on a landing edge observed after the vehicle was airborne."""
        if type(landed) is not bool:
            raise ValueError("landed state must be a boolean")
        if not landed:
            self._airborne_observed = True
            return False
        if not self._airborne_observed:
            return False
        self._airborne_observed = False
        self.stop()
        return True
    ...
    def _align(self, state: RobotState, now_ns: int) -> None:
        ...
        error = (self._sync_target - current + math.pi) % (2.0 * math.pi) - math.pi
