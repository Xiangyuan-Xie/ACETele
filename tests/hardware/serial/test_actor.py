...
from acetele.hardware.serial import actor as actor_module
...
def test_latest_motion_mailbox_replaces_unstarted_target():
    ...
    actor = SerialBusActor(protocol, cycle_hz=100.0)
    actor.connect()
    try:
...
def test_slow_state_read_is_not_starved_by_continuous_motion():
    ...
    actor = SerialBusActor(protocol, cycle_hz=100.0, slow_state_hz=100.0)
    actor.connect()
...
def test_motion_write_and_state_read_respect_period_deadline():
    ...
    actor = SerialBusActor(protocol, cycle_hz=20.0)
    actor.connect()
    try:
